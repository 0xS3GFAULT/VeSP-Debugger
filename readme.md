...
*(Note : I probably found a bug at this line `case 4:   vesp.PC = vesp.IR & 0x1FFF;` in the `void execute(void)` function, where the bit mask should be `0x0FFF` instead of `0x1FFF` according to the documentation where it is written : `JMP  Jump        Opcode: 100  ----- PC = IR[4:15]`. This bug has been patched in my debugger.)* 
